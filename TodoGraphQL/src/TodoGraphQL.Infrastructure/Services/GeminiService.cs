    ...
    public async Task<List<string>> GenerateTodosAsync(string prompt)
    {
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                // Log detalhado — isso vai revelar o motivo real no painel do Render/Console
                _logger.LogError("=== DETALHES DO ERRO GEMINI ===");
                _logger.LogError("Status Code: {Status}", response.StatusCode);
                _logger.LogError("Corpo da Resposta: {ErrorContent}", errorContent);
                _logger.LogError("===============================");

                if (response.StatusCode == System.Net.HttpStatusCode.TooManyRequests)
                {
                    throw new Exception($"Limite da IA atingido. Motivo retornado pelo Google: {errorContent}");
                }
